<template>
  <div>
    ...
    <div style="margin-left: 200px;" v-if="active == 'info'">
      <Form ref="editForm" :model="formData" :rules="ruleValidate" label-position="right" :label-width="90">
        <Row :gutter="16">
          ...
          <Form-item label="房源面积" prop="area">
            <Input v-model="formData.area" placeholder="请输入">
            <span slot="append">平米(㎡)</span>
            </Input>
          </Form-item>
          <Form-item label="房源户型" prop="huxing">
            <Select v-model="formData.huxing" placeholder="请选择">
              <Option value="1">2室1厅</Option>
              <Option value="2">1室1厅</Option>
            </Select>
          </Form-item>
    ...
    <div style="margin-left: 200px;" v-if="active == 'lock'">
      <TableScoller>
        <Table ref="tableB" :columns="columnsB" @on-row-dblclick="goDoorLockEdit" :data="tableLock" stripe border></Table>
      </TableScoller>
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      columnsB: [{
        title: '状态', // 门锁序号, 房源编号, 房源名称, 通讯盒序号, 绑定时间, 撤换时间, 有效日期
        key: 'id',
        render: (h, params) => {
          const row = params.row;
          const isHisotry = row.history == 1;
          console.log(isHisotry)
          return h('Tag', {
            props: {
              type: 'border',
              color: isHisotry ? '' : 'green'
            }
          }, isHisotry ? '历史门锁' : '当前门锁')
        }
      }, {
    ...
    getInfo() {
      this.$lodash.api(this, 'resourceInfo', {
        ...
        this.$store.commit('updateActiveTab', {
          title: '编辑房源:' + this.editInfor.name
        });
      })
    },
    getMembers() {
