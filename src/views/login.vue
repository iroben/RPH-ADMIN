<template>
  <div>
    <Modal class="login-modal" v-model="showLogin" width="360" :closable="false" :mask-closable="false">
      ...
      <div style="text-align:center">
        <Form ref="formData" :model="formData" :rules="ruleValidate">
          ...
          <Form-item prop="account">
            <Input v-model="formData.account" size="large" placeholder="请输入用户名"></Input>
          </Form-item>
          <Form-item prop="password">
            <Input type="password" v-model="formData.password" size="large" placeholder="请输入密码"></Input>
          </Form-item>
          <Form-item prop="password">
      ...
      <div slot="footer">
        <Button :type="loginStaus == 0 ? 'primary' : 'error'" :loading="loginStaus == 1" @click="handleSubmit('formData')" size="large" icon="log-in" long>
          {{loginStaus == 1 ? '登录中...' : '登录'}}
        </Button>
      </div>
    </Modal>
  ...
  data() {
    ...
    methods: {
      searchOrg(query) {
        ...
        this.$store.dispatch('orgList').then(res => {
