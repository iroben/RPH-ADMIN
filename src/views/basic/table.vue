...
export default {
  data() {
    return {
      ...
      columns: [{
        ...
        align: 'center',
        render: (h, params) => {
          return this.$lodash.renderIndex(h, params, this.curPage, this.pageSize);
        }
      }, {
        type: 'selection',
        width: 60,
        ...
        render: (h, params) => {
          const row = params.row;
          return this.$lodash.renderToggle(h, params, {
            active: row.status == 1,
            open: '激活',
            close: '冻结',
            change: (status) => {
        ...
        width: 110,
        render: (h, params) => {
          return this.$lodash.renderDate(h, params, 'start_time');
        }
      }, {
        title: '操作',
        key: 'address',
  ...
  methods: {
    ...
    getData(page, forceReset) {
      ...
      if (Promise) {
        Promise.catch(res => {
          this.tableData = TEST_RESULT.data;
          this.page = TEST_RESULT.page;
        });
      }
    },
    // 批量处理状态
